...
const paginate = async (model, query, options = {}) => {
  ...
  if (options && options.include && options.include.length) {
    ...
    options.include.forEach((i) => {
      if (i.model) {
        let ModelClass = i.model;
        if (models.sequelize.models[ModelClass.name]) {
          i.model = models.sequelize.models[ModelClass.name];
          if (i.query) {
            i.where = queryBuilderParser(i.query);
          }
          include.push(i);
        } else {
          console.error(`Model ${i.model} not found.`);
        }
      } else {
        console.error('Include option is missing the model.');
      }
    });
